...
from LxGeoPyLibs.satellites.formulas import compute_rotation_angle, compute_roof2roof_constants
import math
...
def lazy_ref_epipolar_creation(iref1, input_imd_1, iref2, input_imd_2, i_alt1, i_alt2, epi1, epi2, couple_file):
    """
    ...
    if input_imd_1 and input_imd_2:
        ...
        rotation_angle,_,_ = estimate_epipolarity_properties(iref1=iref1, iref2=iref2)
    
    _logger.info(f"Epipolarity angle = {rotation_angle}")
    
    with rio.open(iref1) as dst:
        ref1_x_size = dst.transform[0]
        ref1_bounds = dst.bounds
        ref1_profile = dst.profile.copy()
    ...
    _logger.info(f"Vertical displacment = {v_disp}")

    xy_cst = compute_roof2roof_constants(math.radians(imd1_obj.satAzimuth()),
         math.radians(imd1_obj.satElevation()),
         math.radians(imd2_obj.satAzimuth()),
         math.radians(imd2_obj.satElevation())
        )
    ...
    detail_dict = {
        "ref1_transform": ref1_profile["transform"].to_gdal(),
        "ref1_width":ref1_profile["width"],
        "ref1_height":ref1_profile["height"],
        "crs":str(ref1_profile["crs"]),
...
@click.option('-c_f', '--couple_file', required=True, type=click.Path(), help="Couple file too be used to reverese disparity")
def main(input_reference_image_1, input_imd_1, input_reference_image_2, input_imd_2,
 input_alternative_image_1, input_alternative_image_2,
  output_epi_image_1, output_epi_image_2, couple_file):
    """
    ...
    lazy_ref_epipolar_creation(input_reference_image_1, input_imd_1, input_reference_image_2, input_imd_2,
     input_alternative_image_1, input_alternative_image_2,
      output_epi_image_1, output_epi_image_2, couple_file
       )
