...
def rotation_respective_profile(in_rotation : inplaceRotationAngle, in_profile: rio.profiles.Profile):
    """
...
def inplace_rotation(rio_dataset : rio.DatasetReader, rotation_angle : inplaceRotationAngle, out_raster_path : str ):
    """
    Applies inplace rotation of a dataset by transforming matrix data & geotransfrom & size
    """
    # Preapre output profile
    out_profile = rotation_respective_profile(rotation_angle, rio_dataset.profile)
...
def main(in_raster_path, out_raster_path, rotation_angle):
    """
    ...
    with rio.open(in_raster_path) as in_rio_dst:
        inplace_rotation(in_rio_dst, rotation_angle, out_raster_path)
